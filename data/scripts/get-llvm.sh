...
if [ ! -d "llvm" ]; then
	...
	cp /c11tester/llvm-8.0.0.src.tar.xz .
...
cp -r /c11tester/cfe-8.0.0.src.tar.xz .
...
mkdir build
cd build
cmake -DCMAKE_BUILD_TYPE=Release -DLLVM_TARGETS_TO_BUILD="X86;ARM" -G Ninja ..
ninja
